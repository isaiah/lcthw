...
struct tagbstring test3 = bsStatic("TET");
...
char *test_search_prefix() {
        ...
        mu_assert(res != NULL, "Should ifnd for short prefix.");
...
char *test_traverse() {
        ...
        mu_assert(traverse_count == 4, "Didn't fidn 4 keys.");
